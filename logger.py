...
class Logger:
    ...
    def log_turn_start(self, player_name: str, game_state: GameState) -> None:
        print(f"{player_name}'s turn")

    def log_action(self, instigator: str, action: Action, target: Optional[str], game_state: GameState) -> None:
        if target is not None:
            print(f"{instigator} claimed {action} against {target}")
        else:
            print(f"{instigator} claimed {action}")

    def log_challenge(self, instigator: str, challenger: str, action: Action, success: bool, game_state: GameState) -> None:
        if success:
    ...
    def log_no_challenge(self, instigator: str, allower: str, action: Action, game_state: GameState) -> None:
        print(f"{allower} did not challenge {instigator} who claimed {action}")

    def log_block(self, instigator: str, blocker: str, action: Action, block_action: Action, game_state: GameState) -> None:
